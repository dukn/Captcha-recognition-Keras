...
class CaptchaBuilder implements CaptchaBuilderInterface
{
     ...
     */
    protected $maxAngle = 2; # 8

    /**
     * The maximum offset of char
     */
    protected $maxOffset = 1; # 5
    ...
    {
        if ($tcol === null) {
            $tcol = imagecolorallocate($image, $this->rand(250, 255), $this->rand(250, 255), $this->rand(250, 255));
        }
    ...
    protected function writePhrase($image, $phrase, $font, $width, $height)
    {
        ...
        $size = $width / $length - $this->rand(0, 3) - 10;
        ...
        if (!count($this->textColor)) {
            $textColor = array($this->rand(0, 50), $this->rand(0, 50), $this->rand(0, 50));
        } else {
    ...
    public function build($width = 150, $height = 40, $font = null, $fingerprint = null)
    {
        ...
        if (true/*empty($this->backgroundImages)*/) {
            // if background images list is not set, use a color fill as a background
            $image   = imagecreatetruecolor($width, $height);
            if ($this->backgroundColor == null) {
                $bg = imagecolorallocate($image, $this->rand(250, 255), $this->rand(250, 255), $this->rand(250, 255));
            } else {
        ...
        // Apply effects
        if (/*!$this->ignoreAllEffects*/ false) {
            $square = $width * $height;
        ...
        // Distort the image
        if (/*$this->distortion && !$this->ignoreAllEffects*/false) {
            $image = $this->distort($image, $width, $height, $bg);
        }

        // Post effects
        if (false/*!$this->ignoreAllEffects*/) {
